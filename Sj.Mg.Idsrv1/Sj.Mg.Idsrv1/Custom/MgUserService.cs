...
namespace Sj.Mg.Idsrv1.Custom
{
    public class MgUserService : UserServiceBase
    {
        ...
        public override Task AuthenticateExternalAsync(ExternalAuthenticationContext context)
        {
            ...
            {
                var claimsList1 = context.ExternalIdentity.Claims.ToList();
                ...
                //claimsList1.Add(new Claim("email", extraClaims.SelectToken("email").ToString()));
                //claimsList1.Add(new Claim("given_name", extraClaims.SelectToken("first_name").ToString()));
                //claimsList1.Add(new Claim("family_name", extraClaims.SelectToken("last_name").ToString()));
                //context.ExternalIdentity.Claims = claimsList1;
            }
            ...
            if (context.ExternalIdentity.Provider == "Yahoo")
            {
                //email = context.ExternalIdentity.Claims.First(x => x.Type == Constants.ClaimTypes.Email).Value;
            }
            ...
            claimsList.Add(new Claim("provider", context.ExternalIdentity.Provider));
            claimsList.Add(new Claim("provider_id", context.ExternalIdentity.ProviderId));
            ...
            else
            {
                var nameClaim = context.ExternalIdentity.Claims.First(x => x.Type == Constants.ClaimTypes.Name);
                if (nameClaim != null) //name = nameClaim.Value;
                    claimsList.Remove(nameClaim);
                claimsList.Add(new Claim(Constants.ClaimTypes.Name, user.GivenName));
            }
            ...
            {
                context.AuthenticateResult = new AuthenticateResult("~/loginchallenge", user.Username, user.GivenName, identityProvider: user.Provider);
                // user is registered so continue
                //context.AuthenticateResult = new AuthenticateResult(user.Username, user.GivenName, identityProvider: user.Provider);
            }
            else
            {
                // user not registered so we will issue a partial login and redirect them to our registration page
                context.AuthenticateResult = new AuthenticateResult("~/externalregistration", context.ExternalIdentity.ProviderId, name, identityProvider: user.Provider);
            }
        ...
        public string addUser(string firstName, string lastName, string password, string email, string phoneNumber, bool provider, string npi, string address, string ans1, string ans2, string ans3, string ans4, string ans5, string ans6, string ans7)
        {
            ...
            if (response.Count == 0)
            {
                try
                {
                    ...
                    filter1.Add("Provider", newUser.Provider);
                    filter1.Add("ProviderID", newUser.ProviderID);
